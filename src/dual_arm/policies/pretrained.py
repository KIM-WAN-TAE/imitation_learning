...
from dual_arm.policies.utils import log_model_loading_keys
...
class PreTrainedPolicy(nn.Module, HubMixin, abc.ABC):
    ...
    def generate_model_card(
        ...
        base_model = "dual_arm/smolvla_base" if model_type == "smolvla" else None  # Set a base model

        card_data = ModelCardData(
            license=license or "apache-2.0",
            library_name="dual_arm",
            pipeline_tag="robotics",
            tags=list(set(tags or []).union({"robotics", "dual_arm", model_type})),
        ...
        template_card = (
            files("dual_arm.templates").joinpath("dual_arm_modelcard_template.md").read_text(encoding="utf-8")
        )
