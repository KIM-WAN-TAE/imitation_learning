...
from dual_arm.utils.constants import ACTION, DONE, OBS_IMAGE, REWARD
from dual_arm.utils.transition import Transition
...
class ReplayBuffer:
    ...
    @classmethod
    def from_dual_arm_dataset(
        cls,
        dual_arm_dataset: LeRobotDataset,
        device: str = "cuda:0",
        state_keys: Sequence[str] | None = None,
        ...
        Args:
            dual_arm_dataset (LeRobotDataset): The dataset to convert.
        ...
        """
        if capacity is None:
            capacity = len(dual_arm_dataset)

        if capacity < len(dual_arm_dataset):
            raise ValueError(
        ...
        list_transition = cls._dual_armdataset_to_transitions(dataset=dual_arm_dataset, state_keys=state_keys)
        ...
        for data in list_transition:
            ...
            replay_buffer.add(
                ...
                truncated=False,  # NOTE: Truncation are not supported yet in dual_arm dataset
                complementary_info=data.get("complementary_info", None),
            )

        return replay_buffer

    def to_dual_arm_dataset(
        self,
        repo_id: str,
        fps=1,
        root=None,
        ...
        dual_arm_dataset = LeRobotDataset.create(
            repo_id=repo_id,
            fps=fps,
            root=root,
            robot_type=None,
        ...
        dual_arm_dataset.start_image_writer(num_processes=0, num_threads=3)
        ...
        for idx in range(self.size):
            ...
            dual_arm_dataset.add_frame(frame_dict)

            # If we reached an episode boundary, call save_episode, reset counters
            if self.dones[actual_idx] or self.truncateds[actual_idx]:
                dual_arm_dataset.save_episode()

        # Save any remaining frames in the buffer
        if dual_arm_dataset.episode_buffer["size"] > 0:
            dual_arm_dataset.save_episode()

        dual_arm_dataset.stop_image_writer()
        dual_arm_dataset.finalize()

        return dual_arm_dataset

    @staticmethod
    def _dual_armdataset_to_transitions(
