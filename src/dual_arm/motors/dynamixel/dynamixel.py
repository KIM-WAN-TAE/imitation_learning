...
DEFAULT_TIMEOUT_MS = 100 # 1000ms에서 100ms로 단축
...
class DynamixelMotorsBus(SerialMotorsBus):
    ...
    def reboot(self, motor: NameOrID):
        import dynamixel_sdk as dxl
        id_ = self._get_motor_id(motor)
        proto = self._select_protocol_for_motor(id_)
        with self._with_protocol(proto):
            self.packet_handler.reboot(self.port_handler, id_)
        logger.info(f"Motor {id_} rebooted.")
    ...
    def _write(
        ...
        *,
        ...
        """
        AX-12A(Protocol 1.0) 및 X-series(Protocol 2.0)에 대해 명시적인 writeNByteTxRx 함수를 호출합니다.
        """
        if length == 1:
            write_fn = self.packet_handler.write1ByteTxRx
        elif length == 2:
            write_fn = self.packet_handler.write2ByteTxRx
        elif length == 4:
            write_fn = self.packet_handler.write4ByteTxRx
        else:
            raise ValueError(f"Unsupported byte length: {length}")

        for n_try in range(1 + num_retry):
            comm, error = write_fn(self.port_handler, motor_id, addr, value)
            if self._is_comm_success(comm):
                break
            logger.debug(
