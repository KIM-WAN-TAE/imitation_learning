...
```shell
dual_arm-setup-can --mode=setup --interfaces=can0,can1,can2,can3
```

Test motors on a single interface:
```shell
dual_arm-setup-can --mode=test --interfaces=can0
```

Test motors on all interfaces:
```shell
dual_arm-setup-can --mode=test --interfaces=can0,can1,can2,can3
```

Speed test:
```shell
dual_arm-setup-can --mode=speed --interfaces=can0
```
"""
...
from dual_arm.utils.import_utils import _can_available
...
def test_interface(cfg: CANSetupConfig, interface: str):
    ...
    if not is_up:
        print(f"  ⚠ Interface is not UP. Run: dual_arm-setup-can --mode=setup --interfaces {interface}")
        return {}
...
def run_setup(cfg: CANSetupConfig):
    ...
    print(f"  dual_arm-setup-can --mode=test --interfaces {','.join(interfaces)}")
...
def run_test(cfg: CANSetupConfig):
    ...
    if total_found == 0:
        ...
        print(f"  5. Interface configured: dual_arm-setup-can --mode=setup --interfaces {interfaces[0]}")
