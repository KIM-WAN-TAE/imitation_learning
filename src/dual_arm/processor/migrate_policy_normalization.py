...
Usage:
    python src/dual_arm/processor/migrate_policy_normalization.py \
        --pretrained-path dual_arm/act_aloha_sim_transfer_cube_human \
        --push-to-hub \
        --branch main

Note: This script now uses the modern `make_pre_post_processors` and `make_policy_config`
factory functions from `dual_arm.policies.factory` to create processors and configurations,
...
from dual_arm.configs.types import FeatureType, NormalizationMode, PolicyFeature
from dual_arm.policies.factory import get_policy_class, make_policy_config, make_pre_post_processors
...
def display_migration_summary_with_warnings(problematic_missing_keys: list[str]) -> None:
    ...
    print("     https://github.com/huggingface/dual_arm/issues")
...
def load_model_from_hub(
    ...
    Args:
        repo_id: The repository ID on the Hub (e.g., 'dual_arm/aloha').
...
def main():
    ...
    tags = config.get("tags", ["robotics", "dual_arm", policy_type]) or ["robotics", "dual_arm", policy_type]
    tags = set(tags).union({"robotics", "dual_arm", policy_type})
    tags = list(tags)
...
from dual_arm.policies import get_policy_class
from dual_arm.processor import PolicyProcessorPipeline
