...
from dual_arm.configs.types import PolicyFeature
from dual_arm.datasets.utils import build_dataset_frame, hw_to_dataset_features

# NOTE: Configs need to be loaded for the client to be able to instantiate the policy config
from dual_arm.policies import (  # noqa: F401
    ACTConfig,
    DiffusionConfig,
    PI0Config,
    PI05Config,
...
from dual_arm.robots.robot import Robot
from dual_arm.utils.constants import OBS_IMAGES, OBS_STATE, OBS_STR
...
def map_robot_keys_to_dual_arm_features(robot: Robot) -> dict[str, dict]:
...
def raw_observation_to_observation(
    ...
    dual_arm_features: dict[str, dict],
    policy_image_features: dict[str, PolicyFeature],
) -> Observation:
    observation = {}

    observation = prepare_raw_observation(raw_observation, dual_arm_features, policy_image_features)
...
def extract_state_from_raw_observation(
    dual_arm_obs: RawObservation,
) -> torch.Tensor:
    """Extract the state from a raw observation."""
    state = torch.tensor(dual_arm_obs[OBS_STATE])
...
def extract_images_from_raw_observation(
    ...
    return torch.tensor(dual_arm_obs[camera_key])


def make_dual_arm_observation(
    robot_obs: RawObservation,
    dual_arm_features: dict[str, dict],
) -> LeRobotObservation:
    """Make a dual_arm observation from a raw observation."""
    return build_dataset_frame(dual_arm_features, robot_obs, prefix=OBS_STR)
...
def prepare_raw_observation(
    ...
    dual_arm_obs = make_dual_arm_observation(robot_obs, dual_arm_features)

    # 2. Greps all observation.images.<> keys
    image_keys = list(filter(is_image_key, dual_arm_obs))
    # state's shape is expected as (B, state_dim)
    state_dict = {OBS_STATE: extract_state_from_raw_observation(dual_arm_obs)}
    image_dict = {
        image_k: extract_images_from_raw_observation(dual_arm_obs, image_k) for image_k in image_keys
    }

    # Turns the image features to (C, H, W) with H, W matching the policy image features.
    # This reduces the resolution of the images
    image_dict = {
        key: resize_robot_observation_image(torch.tensor(dual_arm_obs[key]), policy_image_features[key].shape)
...
class RemotePolicyConfig:
    ...
    dual_arm_features: dict[str, PolicyFeature]
...
def observations_similar(
    obs1: TimedObservation, obs2: TimedObservation, dual_arm_features: dict[str, dict], atol: float = 1
) -> bool:
    ...
    obs1_state = extract_state_from_raw_observation(
        make_dual_arm_observation(obs1.get_observation(), dual_arm_features)
    )
    obs2_state = extract_state_from_raw_observation(
        make_dual_arm_observation(obs2.get_observation(), dual_arm_features)
    )
