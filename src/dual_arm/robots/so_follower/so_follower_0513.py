...
class SOFollower(Robot):
    ...
    "elbow_roll": 2958, "wrist_yaw": 2048, "wrist_roll": 2048, "wrist_pitch": 2958, "gripper": 2048,
        ...
        self._all_acc_values = {name: config.profile_accelerations for name in self._motor_names}
        self._home_vel_values = {name: config.home_profile_velocities for name in self._motor_names}
        self._work_vel_values = {name: config.work_profile_velocities for name in self._motor_names}
    ...
    def _expand_motor_values(value, motor_names, default):
    ...
    def _load_calibration_from_json(self) -> None:
        """
        기존 코드의 여러 경로 탐색을 제거하고, config.calibration_path를 우선 사용.
        """
        ...
        with self.bus.torque_disabled():
            ...
            for motor in self.bus.motors:
                ...
                if motor_id in self.EXTENDED_POSITION_MOTOR_IDS:
                    ...
                    self._all_acc_values = _expand_motor_values(
                        config.profile_accelerations,
                        self._motor_names,
                        200,
                    )

                    self._home_vel_values = _expand_motor_values(
                        config.home_profile_velocities,
                        self._motor_names,
                        1000,
                    )

                # self.bus.write("Profile_Acceleration", motor, 200, normalize=False)
                # self.bus.write("Profile_Velocity", motor, 1000, normalize=False)
    ...
    def get_observation(self) -> RobotObservation:
        ...
        if not self.config.obs_skip_cameras:
            for cam_key, cam in self.cameras.items():
                obs_dict[cam_key] = cam.read_latest(max_age_ms=self.config.camera_max_age_ms)
    ...
    def send_action(self, action: RobotAction) -> RobotAction:
        ...
        for key, val in action.items():
            ...
            corrected_val = init_follow_pos + ((raw_val - home_pos) * ratio * direction)
